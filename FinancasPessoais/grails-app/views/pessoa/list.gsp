			...
			<table>
				<thead>
					<tr>
						...
						<g:sortableColumn property="salario" title="${message(code: 'pessoa.salario.label', default: 'Salario')}" />
					
						<g:sortableColumn property="sitFinanceira" title="${message(code: 'pessoa.sitFinanceira.label', default: 'Sit Financeira')}" />
				...
				<g:each in="${pessoaInstanceList}" status="i" var="pessoaInstance">
					<tr class="${(i % 2) == 0 ? 'even' : 'odd'}">
						...
						<td><g:link action="show" id="${pessoaInstance.id}">${fieldValue(bean: pessoaInstance, field: "endereco")}</g:link></td>
					
						<td>${fieldValue(bean: pessoaInstance, field: "nome")}</td>
					
						<td>${fieldValue(bean: pessoaInstance, field: "salario")}</td>
					
						<td>${fieldValue(bean: pessoaInstance, field: "sitFinanceira")}</td>
